...
| ✅ | MSX 账户管理 | 支持 MSX 账户 CRUD、凭证加密存储、连接测试、启用/禁用、删除和账户汇总。 |
| ✅ | 网格策略管理 | 支持策略创建、启动、暂停、恢复、停止、删除、参数重配、状态持久化和控制台页面。 |
| ✅ | 订单服务 | 支持统一订单提交、撤单、订单审计、模拟/实盘开关保护，并已接入网格策略挂单/撤单流程。 |
| ⬜ | 网格实盘执行 | 接入订单服务后完成真实挂单、成交回补、网格闭环收益和异常恢复。 |
